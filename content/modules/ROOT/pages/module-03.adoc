...
[source,bash,role="execute",subs=attributes+]
----
podman image scp lab-user@localhost::registry-{guid}.{domain}/httpd
----
...
After the image builder version, these arguments passed to `bootc-image-builder`:
  ...
  * `registry-{guid}.{domain}/httpd` -> the bootc image we are unpacking into the qcow2 disk
...
[source,bash,role="execute",subs=attributes+]
----
skopeo inspect docker://registry-{guid}.{domain}/httpd | jq '.Digest'
----
....
