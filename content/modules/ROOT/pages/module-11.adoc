...
Now update the Containerfile to use the new structure.
[source,bash,role="execute",subs=attributes+]
----
nano Containerfile
----

Find the following block in the current Containerfile and replace it with the new block that follows:
[source,dockerfile,role="execute",subs=attributes+]
----
# Copy the Flask application files and Gunicorn service file
COPY . /app
...
Finally, remove the `heredoc` near the end of the file we created for testing the `nginx` workaround. 
