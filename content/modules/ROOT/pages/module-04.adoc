...
We've written more about this '3 way merge' process[https://developers.redhat.com/articles/2025/08/25/what-image-mode-3-way-merge,window=_blank] and the implications for image mode hosts.
[source,bash,role="execute",subs=attributes+]
----
...
....
NOTE: At the time of writing, the first update of an image mode host may be large. This is due to registry versus local storage when the disk image was created.
...
[#testing]
== Testing the changes
Log back into the VM and look for the kernel arguments we added via the `bootc` configuration path.
[source,bash,role="execute",subs=attributes+]
----
cat /proc/cmdline
----
